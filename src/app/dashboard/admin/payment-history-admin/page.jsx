...
const PaymentHistoryAdmin = () => {
	...
	useEffect(() => {
		const fetchPayments = async () => {
			try {
				const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/dashboard/get-payments`,
					{ cache: "no-store" }
				);
								...
								payments?.map((payment, index) => (
									<tr key={payment?._id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
										...
										<td className="px-2 py-2 text-center uppercase border-r-2">
											{payment?.payment_status === true ? "Paid" : "Unpaid"}
										</td>
										<td className="px-2 py-2 text-center uppercase border-r-2">
											{payment?.createdAt && (
												<div>
													{formatTimestamp(payment.createdAt)[0]} {/* Date */}
													<br />
													{formatTimestamp(payment.createdAt)[1]} {/* Time */}
												</div>
											)}
										</td>
										<td className="px-2 py-2 text-center uppercase border-r-2">
											{payment?.updatedAt && (
												<div>
													{formatTimestamp(payment.updatedAt)[0]} {/* Date */}
													<br />
													{formatTimestamp(payment.updatedAt)[1]} {/* Time */}
												</div>
											)}
										</td>
									</tr>)
