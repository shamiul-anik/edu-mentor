...
const FeedbackModal = ({ isOpen, closeModal, feedbackID, processing, setProcessing }) => {
	...
	const onSubmit = (feedback) => {
		...
		// console.log("Feedback Message: ", feedback.tutor_feedback);
		const tutor_feedback = feedback.tutor_feedback;

		const fetchFeedback = async () => {
			setProcessing(true);
			try {
				const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/dashboard/tutor/my-inbox?id=${feedbackID}&feedback=${tutor_feedback}`, {
					method: "PATCH",
					cache: "no-store"
				});
							...
							<Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
								...
								<div className="mt-2 border-t">
									<div className="form-control mt-3">
										<label className="label pl-0" htmlFor="tutorFeedback">
											<span className="label-text text-md md:text-[16px]">Message</span>
										</label>
										<textarea id="tutorFeedback" {...register("tutor_feedback", { required: true })} name="tutor_feedback" rows="6" className="input input-bordered focus:outline-slate-500 focus:border-slate-200 focus:ring-slate-400 text-[14px] h-auto" placeholder="Enter your message"></textarea>
										{errors?.tutor_feedback && <p className="text-red-500 mt-2">Message is required!</p>} {/* Error Message */}
									</div>
								</div>
