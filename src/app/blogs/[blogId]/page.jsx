            ...
            <CommonBanner bannerHeading="Post Details"></CommonBanner>
            {
                blog.map((item, index) => <div key={index}  >
                    ...
                    <div className=" lg:w-[80%] w-[95%] mx-auto shadow-lg lg:p-8 p-4 rounded-md bg-slate-100 shadow-slate-500 items-center">
                        ...
                        <div className="my-4">
                            <Image className='object-cover max-sm:mx-auto max-md:mx-auto mx-2 my-4 rounded-md' src={item.postImgUrl} alt='' width={800} height={400} />
                            <p>{item.postDescription}</p>
                        </div>
                        <p>Total Comment: <span className='font-bold text-lg'>{comments.length}</span></p>
                    </div>
                </div>)
            }
            <div className=" border border-5 mt-4 border-slate-600 lg:w-[80%] w-[95%] mx-auto shadow-lg lg:p-8 p-4 rounded-md bg-slate-100 shadow-slate-500 ">
                <div className="lg:flex md:flex ">
                    <div className="mx-1 ">
                        {
                            user ?
                                <Image className='object-cover m-1 rounded-full' src={user?.photoURL} alt='' width={40} height={40} />
                                :
                                <FaUserTimes className="w-10 h-10 rounded-full p-1"></FaUserTimes>
                        }
                    </div>
                    <form className='w-[95%] ' onSubmit={handleSubmit(onSubmit)}>
                        <textarea className='w-[100%] h-[100px] rounded-md' placeholder="Comment" {...register("blogComment", {})} />
                        {
                            user == null ?
                                <Link href={"/login"}>
                                    <button className='bg-slate-600 p-2  ml-1 rounded-md my-2 text-white hover:bg-slate-700' >Submit</button>
                                </Link>
                                :
                                <button className='bg-slate-600 p-2  ml-1 rounded-md my-2 text-white hover:bg-slate-700' type='submit'>Submit</button>
                        }
                    </form>
                </div>
